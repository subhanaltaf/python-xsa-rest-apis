...
function applyRewriteRule(req, route, routerConfig) {
  ...
  var useHtml5AppsRepo = route.service && route.service === 'html5AppsRepo' ? true : false;
  var path;
  ...
  }  else if (useHtml5AppsRepo){
    var credentials = xsenv.cfServiceCredentials({tag: 'html5-apps-repo-rt'});
    var oHTML5AppRepoDestination = { 'url' : credentials.uri + '/' };
    rewrittenUrl = urlUtils.parse(_.trimEnd(oHTML5AppRepoDestination.url, '/') + '/' + _.trimStart(path, '/'));
    rewrittenUrl.destination = oHTML5AppRepoDestination;
  } else {
