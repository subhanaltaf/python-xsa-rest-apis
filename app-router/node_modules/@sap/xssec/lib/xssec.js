    ...
    offlineValidation(
            ...
            function(error, result) {
                ...
                if ((result.cid === self.config.clientid)
                        && (result.zid === self.config.identityzoneid || result.zid === self.config.identityzone || applicationPlan === true)) {
                ...
                } else if (self.config.trustedclientidsuffix) {
                    debugTrace('\nToken of UAA service plan "broker" received.\n');
                    if (self.config.trustedclientidsuffix.length > 0 &&
                        ...
                        var errorString = '\nClient Id "' + result.cid + '" of the access token does not allow consumption by the Client Id "' +
                                self.config.clientid + '" of the current application\n';
                        debugError('\n' + errorString);
                        var error = new Error(errorString);
                        error.statuscode = 403; //403 Forbidden (as no trustedclientidsuffix)
                        return cb(error, null);
                    }
                    ...
                    if (result.zid !== self.config.identityzoneid && result.zid !== self.config.identityzone) {
                ...
                debugTrace('\nObtained serviceinstanceid: ' + self.serviceinstanceid);
