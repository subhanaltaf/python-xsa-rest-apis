...
const DEFAULT_DATA_DIR = 'db'
const DEFAULT_SERVICE_DIR = 'srv'
...
function build([ project ], options={}, config) {
  ...
  const _local = (filename) => path.resolve(project, filename)
  ...
  return Promise.all([
    cds.resolve (config.data && config.data.model || DEFAULT_DATA_DIR),
    cds.resolve (config.service && config.service.model || DEFAULT_SERVICE_DIR)
  ])
  .then(([dataModels, serviceModels]) => {
    ...
    // to HANA
    if (dataModels) {
      if (dataModels.length > 1)  throw new Error(
        `Multiple models found: [${toRelPaths(dataModels)}].  Name one 'index.cds|json' or add configuration.`
      )
      for (let modelPath of dataModels) {
        ...
        if (serviceModels && serviceModels.length > 0 && data.includeServiceViews !== false) {
        ...
        const p = compile(models.map(_local), { to: 'hana', sql_mapping, extraArtifacts: true, dest: outDir, log })
    ...
    // to OData
    if (serviceModels) {
      for (let modelPath of serviceModels) {
        ...
        outputPaths.push(outDir)
        const serviceModelPathFull = _local(modelPath)
        const log = file => log.files.push(file); log.files = []
        const baseOptions = {}
        ...
        const p = Promise.all([
          compile(serviceModelPathFull, odataCsnOptions),
          compile(serviceModelPathFull, edmxOptions)
...
function validate(dataModels, serviceModels, config, project) {
  if (!dataModels && !serviceModels) {
    throw new Error(`No models found in '${project}' and configuration '${config._source || JSON.stringify(config)}'.  Use \`cds init <project>\` to create a project.`)
  }
  if (!dataModels && config.data && config.data.model && config.data.model !== '--') {
    throw new Error(`No models found for '${config.data.model}' as configured in '${config._source || JSON.stringify(config)}'`)
  }
  if (!serviceModels && config.service && config.service.model && config.service.model !== '--') {
    throw new Error(`No models found for '${config.service.model}' as configured in '${config._source || JSON.stringify(config)}'`)
  }
}
...
function displayErrors (e) {
  if (Array.isArray(e.errors)) { // errors from CDSV
    e.errors.forEach(err => console.error(term.decorate.error(err)))
    return Promise.resolve(1) //  exit code
  }
  // own/other errors
