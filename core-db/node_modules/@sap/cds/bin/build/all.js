...
module.exports = Object.assign ( build, {
    options: ['--project', '--module', '--dest', '--dest', '--lang', '--sql' ], flags: ['--clean', '--incremental', '--dry' ],
...
function build (folders=[], o={}, /* for unit tests only: */ _conf) {
    ...
    if (!build.all) return Promise.reject (
        "No modules to build found in '"+ cwd +"'.  Use `cds init` to add some."
    )
    ...
    function _build (options) {
        ...
        const clean = (
            options.clean === false || o.incremental  ?  Promise.resolve()
            :  fs.rimraf (dest,'if-exists')
        )
        const clone = ()=>{
