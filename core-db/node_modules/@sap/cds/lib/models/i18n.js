...
const I18nFolder1 = '_i18n'
const I18nFolder2 = 'i18n'
const I18nFile1 = 'i18n'
const CachedFolders = {}
...
function* bundles4 (model, languages) {
  ...
  if (!languages || languages === '*' || languages === 'all') {
    ...
    const files = folders
      .map(folder => fs.readdirSync(folder).filter(f => f.startsWith(I18nFile1)))
...
function _bundle4(lang, folders) {
  ...
  for (let each of _folders) {
    ...
    if (lang === DefaultLanguage) {
      current.__proto__ = bundle    // put previous (default) bundle below this default bundle
    }
    else {
      let bDefault = loadBundleFrom(each, DefaultLanguage);  if (!bDefault) continue
      current.__proto__ = bDefault  // put default bundle below current one
      bDefault.__proto__ = bundle  // put previous below default bundle
    }
    bundle = current
  }
...
function folder4 (loc) {
  ...
  if (fs.existsSync (path.join (loc, I18nFolder1))) {
    return CachedFolders[loc] = path.join (loc, I18nFolder1)
  }
  if (fs.existsSync (path.join (loc, I18nFolder2))) {
    return CachedFolders[loc] = path.join (loc, I18nFolder2)
  } else {
...
function loadBundleFrom (folder, lang) {
  ...
  let file = path.join (folder, I18nFile1)
  let bundle = (
    // prefer .properties...
