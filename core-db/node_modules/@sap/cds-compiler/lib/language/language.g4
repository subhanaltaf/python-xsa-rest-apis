...
@after { $source.namespace = this.attachLocation($decl); }
  :
    ...
    |
      '::'
      { $decl.dcPath = $decl.path; delete $decl.path; }
      simplePath[ $decl ]?
      ';'
    )
  |
...
@after { this.attachLocation($query); }
  :
    ...
    |
      // epsilon
      {
        $query.all = { value: true, location: $src.location };
...
@after { this.attachLocation($art); }
  :
    ...
    { if (!this.options.tntFlavor || this.options.tntFlavor.skipExtendingProjections) {
...
@after { this.attachLocation($query); }
  ...
  |
    ...
    ( all=ALL { this.notYet('ALL', $all, ['hanaFlavor']); }
    | distinct=DISTINCT { $query.distinct = this.tokenLocation( $distinct, undefined, true ); }
    )?
    '{'
    ( star='*'
      {
...
@after{
  ...
  |
    expr=expressionOrAspectExpression // see @after
    (
      rel=( '=' | '<>' | '>'  | '>=' | '<' | '<=' | '!=' )
      ...
      e2=expressionOrAspectExpression { $cond.args.push($e2.expr); }
    |
...
//    NOT (a BETWEEN b AND c)
  :
    ino=IN e1=expressionOrAspectExpression // including ExpressionList
  ...
  |
    lk=LIKE e4=expressionOrAspectExpression
...
// conditionOrExpression returns [ cond ] locals [ args = [] ]
// // should only be followed by ',' or ')'
// @after{
//   $cond = ($args.length == 1)
//     ? $args[0]
//     : this.attachLocation({ op: this.tokenLocation( $cond, undefined, 'or' ), args: $args });
// }
//   :
//     c1=conditionAndOE { $args.push($c1.cond); }
//     ( OR c2=conditionAnd { $args.push($c2.cond); } )*
//   ;

// conditionAndOE returns [ cond ] locals [ args = [] ]
// @after{
//   $cond = ($args.length == 1)
//     ? $args[0]
//     : this.attachLocation({ op: this.tokenLocation( $cond, undefined, 'and' ), args: $args });
// }
//   :
//     c1=conditionTermOE { $args.push($c1.cond); }
//     ( AND c2=conditionTerm { $args.push($c2.cond); } )*
//   ;

// conditionTermOE returns [ cond ]
// @after{
//   if ($cond) { this.attachLocation($cond); } else { $cond = $expr.expr; }
// }
//   :
//     NOT ct=conditionTerm
//     { $cond = { op: this.tokenLocation( $ct, undefined, 'not' ), args: [ $ct.cond ] }; }
//   |
//     EXISTS '(' qe=queryExpression ')'
//     { $cond = { op: this.tokenLocation( $qe, undefined, 'exists' ), args: [ $qe.query ] }; }
//   |
//     expr=expressionOrAspectExpression // see @after
//     (
//       rel=( '=' | '<>' | '>'  | '>=' | '<' | '<=' | '!=' )
...
//       { $cond = { args: [ $expr.expr ] }; }
//       predicate[ $cond, false ]
//     )?                        // optional: for expression in parentheses
//   ;

expressionOrAspectExpression returns [ expr ]
  :
    // This ASPECT expression syntax is less than optimal: we need to reserve
    // the keyword ASPECT (even though it is not reserved in SQL), otherwise
    // the parsing of every expression would be far slower (not LL1 anymore).
    // Probably better: get rid of it in normal expressions, and restrict slow
    // parsing to dcl stuff, or even in CDx: ASPECT '(' queryPath ')'
    as=ASPECT path=queryPath
    { $expr = { op: this.tokenLocation( $as, undefined, 'special' ), location: $path.qp.location };
      this.notYet('ASPECT in expressions', $as, ['hanaFlavor']); }
  |
    ...
    |
      ...
      (
        ...
        ( ow=WHEN ew=expression THEN e3=expression
          { $expr.args.push( { op: this.tokenLocation( $ow, undefined, 'when' ), args: [ $ew.expr, $e3.expr ] } ); }
        )+
      |
        ( ow=WHEN c=condition THEN e3=expression
          { $expr.args.push( { op: this.tokenLocation( $ow, undefined, 'when' ), args: [ $c.cond, $e3.expr ] } ); }
        )+
      )
      ( el=ELSE e4=expression
        { $expr.args.push( { op: this.tokenLocation( $el, undefined, 'else' ), args: [ $e4.expr ] } ); }
      )?
      END
    |
      ne=NEW nqp=queryPath      // NEW not reserved in SQL-92
      { $expr = { op: this.tokenLocation( $ne, undefined, 'new' ) };
...
@after{ /* #ATN 1 */ }
  :
    tr=TRIM '('
    { this.notYet('TRIM in expressions', $tr, ['hanaFlavor']); $ret.op = this.tokenLocation( $tr, undefined, 'special' ) }
  ...
  |
    ex=EXTRACT '('
    { this.notYet('EXTRACT in expressions', $ex, ['hanaFlavor']); $ret.op = this.tokenLocation( $ex, undefined, 'special' ) }
    ( YEAR | MONTH | DAY | HOUR | MINUTE | SECOND )
    FROM expression
    ')'
  |
    ca=CAST '('
    { this.notYet('CAST in expressions', $ca, ['hanaFlavor']); $ret.op = this.tokenLocation( $ca, undefined, 'special' ) }
...
queryPathForGrant
@after{ /* #ATN 2 */ }
// extra rule, because ambiguities caused by '.' SELECT syntax should not creep
// into all other uses of `queryPath`
  :
    // #ATN: ambiguity with '.' SELECT, in loop (2x)
    ident ( '.' ident )*
  ;
...
@after{ /* #ATN 1 */ }
  :
    ...
    co=condition { $pathStep.filter = $co.cond; }
    ']'
  ;
...
ANY : [aA][nN][yY] ;
AS : [aA][sS] ;
ASPECT : [aA][sS][pP][eE][cC][tT] ; // not reserved in SQL-92
BY : [bB][yY] ;
CASE : [cC][aA][sS][eE] ;
CAST : [cC][aA][sS][tT] ;
...
IN : [iI][nN] ;
KEY : [kK][eE][yY] ;
NEW : [nN][eE][wW] ;            // not reserved in SQL-92
NOT : [nN][oO][tT] ;
NULL : [nN][uU][lL][lL] ;
OF : [oO][fF] ;
ON : [oO][nN] ;
