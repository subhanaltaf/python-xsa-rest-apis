...
// }
function toSqlDdl(model, options) {
  const { error, signal, getErrors } = alerts(model);

  // Merge options (arguments first, then model options)
  options = mergeOptions(model.options, options);
  let flatNames = options.toSql && options.toSql.names == 'flat';
  ...
  // FIXME: Currently requires 'options.toHana', because it can only render HANA-ish SQL dialect
  if (!options.toHana) {
  ...
  function renderEntity(art, env) {
    ...
    let result = env.indent + 'CREATE ' + storeType + 'TABLE ' + quoteSqlId(art.name.absolute);
    result += ' (\n';
  ...
  function renderElement(elm, env) {
    // Ignore if toHana says so, or if it is an association
    if (elm._ignore) {
      return '';
    }
  ...
  function renderAssociationElement(elm, env) {
    ...
    if (isAssociation(elm._finalType.type)) {
      ...
      result += ' JOIN ';
      result += quoteSqlId(elm.target.absolute) + ' AS ' + quoteSqlId(elm.name.id) + ' ON (';
  ...
  function renderProjection(art, env) {
    let childEnv = increaseIndent(env);
    let result = env.indent + 'CREATE VIEW ' + quoteSqlId(art.name.absolute) + ' AS SELECT\n';
  ...
  function renderSourcePathWithAlias(source, art) {
    ...
    let result = source.path[0]._artifact.name.absolute;
    ...
    for (let i = 1; i < source.path.length; i++) {
      if (source.path[i]._artifact && source.path[i]._artifact.kind != 'element') {
        ...
        result += '.' + source.path[i]
      } else {
        // Path continues with elements - complain if it has a filter
        if (source.path[i].filter) {
          signal(error`"${art.name.absolute}": Filters in FROM are not supported for conversion to SQL`, source.location);
          return '';
        }
        if (result.startsWith('"')) {
          // We are already in a path - append '.' and quoted id
  ...
  function renderView(art, env) {
    let result = env.indent + 'CREATE VIEW ' + quoteSqlId(art.name.absolute)
  ...
  function renderQuery(query, art, isLeadingQuery, env) {
    ...
    if(Array.isArray(query)) {
      ...
      let childEnv = increaseIndent(env);
      result += 'SELECT' + ((query.distinct && query.distinct.val) ? ' DISTINCT\n' : '\n');
  ...
  function renderType(art, env) {
    ...
    let result = env.indent + 'CREATE TYPE ' + quoteSqlId(art.name.id) + ' AS TABLE (\n';
    let childEnv = increaseIndent(env);
  ...
  function renderPath(path, env, art) {
    ...
    if (options.toHana && path[0].id === '$projection') {
    ...
    if (options.toHana && path[0].id === '$self' && path.length > 1) {
      return `${quoteSqlId(art.absolute)}.${renderPath(path.slice(1), env)}`
    }
  ...
  // Return a name 'name' with appropriate "-quotes
  ...
  function quoteSqlId(name) {
    return '"' + (flatNames ? name.toUpperCase() : name).replace(/"/g, '""') + '"';
  }
}
