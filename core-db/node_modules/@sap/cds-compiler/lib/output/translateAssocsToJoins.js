  ...
  */
  function mergePathIntoQAT(pathDict, env)
  {
    ...
    for(let pathStep of tail)
    {
      ...
      let filterStr = '';
      if(pathStep.filter)
        filterStr = JSON.stringify(compactCondOrExpr(pathStep.filter));
      ...
      if (!qat) 
      {
        ...
        if(pathStep.filter)
          setProp(qat, '_filter', pathStep.filter);
        qat.kind = '$navElement';
...
function walkPath(node, env)
{
  ...
  if(path && path[0]._artifact)
  {
    ...
    if(filterEnv.walkover[filterEnv.location])
    {
      ...
      path.filter(pathStep=>pathStep.filter).forEach(pathStep => {
        filterEnv.pathStep = pathStep;
        walk(pathStep.filter, filterEnv) });
    }
