const schemaObjects = require('./schemaObjects');
const { forEachDefinition } = require('../base/model');
const preprocessModel = require('./preprocessModelForSwagger');
const { transformTntExtensions } = require('../edm/tntSpecific');
let swaggerJson = null;

function csnToSwagger(model) {
  ...
  forEachDefinition(model, obj => {
    ...
    if (obj.kind === 'service') {
      ...
      Object.keys(obj.artifacts).forEach(artName => {
        ...
        if (art.actions)
          Object.keys(art.actions).forEach(actName => Object.assign(swaggerJson.paths, createPath(actName, art.actions[actName])));

        // unbound function or action
        if (art.kind === 'function' || art.kind === 'action') {
          Object.assign(swaggerJson.paths, createPath(artName, art));
      ...
      result[obj.name.absolute] = swaggerJson;
    }
  })
  return result;
}
...
function createPath(actName, action) {
  ...
  function getPathString(action) {
    ...
    return `/${action.name.absolute.slice(action.name.absolute.lastIndexOf('.') + 1)}${action.name.action ? '/' + action.name.action : ''}`;
  }
}
