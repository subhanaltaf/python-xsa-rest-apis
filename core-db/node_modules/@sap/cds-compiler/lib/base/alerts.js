...
//   getAlerts(severity)                // Return all messages with 'severity', which can be ('Error', Warning', 'Info')
//   getErrors()                        // Shorthand for getAlerts('Error')
// }
...
function alerts(model) {
  ...
  return {
    ...
    getAlerts, getErrors                // get all alerts resp. only errors
  };
  ...
  // Get all errors
  function getErrors() {
    return getAlerts('Error');
  }

  // Get all collected alerts. Alerts may be filtered if one ore more severities are provided via 'filter'.
  // E.g. filter = [ 'Error', 'Info' ] returns all errors and infos.
  function getAlerts(filter) {
    if (!filter) return collected;
    let f = filter instanceof Array ? filter : [ filter ];
    return collected.filter(e => (f.indexOf(e.severity) >= 0));
  }
