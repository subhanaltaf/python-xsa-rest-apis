...
function preprocessAnnotations(csn, options) {
    ...
    function handleAnnotations(carrierName, carrier) {
      ...
      for (let aName of annoNames) {
        ...
        if (aNameWithoutQualifier == "@Common.ValueList.entity"
            && !(options && options.tntFlavor && options.tntFlavor.skipAnnosFixedValueListShortcut)) {
          try {
        ...
        // TNT - HeaderInfoTitleExpansion
        //   annotation is structured according to vocabulary, but
        //     in the csn a simple value is provided
        if (aNameWithoutQualifier == "@UI.HeaderInfo.Title" &&
            (options && options.tntFlavor && !options.tntFlavor.skipAnnosHeaderInfoTitleExpansion)) {
          carrier[aName] = { "Value": { "=": a["="] } }
        }

        // TNT - UIIdentificationExpansion
        //   annotation is structured according to vocabulary, but
        //     in the csn a simple value is provided
        if (aNameWithoutQualifier == "@UI.Identification" &&
            (options && options.tntFlavor && !options.tntFlavor.skipAnnosUIIdentificationExpansion)) {
          a.forEach( function(x, i, A) {
            if (x["="]) A[i] = { "Value": { "=": x["="] } }; });
        }

        // TNT - VisualizationsSimplification
        //   remove unnecessary nesting
        if (aNameWithoutQualifier == "@UI.PresentationVariant.Visualizations" &&
            (options && options.tntFlavor && !options.tntFlavor.skipAnnosVisualizationsSimplification)) {
          a.forEach( function(x, i, A) {
            if (x["$AnnotationPath"]) A[i] = x["$AnnotationPath"]; });
        }

        // TNT - SortOrderSimplification
        //   remove unnecessary nesting
        if (aNameWithoutQualifier == "@UI.PresentationVariant.SortOrder" &&
            (options && options.tntFlavor && !options.tntFlavor.skipAnnosSortOrderSimplification)) {
          for (let i in a) {
            let ae = a[i];
            let recA = ae["$Record"];
            if (recA != undefined) {
              for (let recEntry of recA) {
                let pv = recEntry["$PropertyValue"];

                if (pv["Property"] == "Property") {
                  ae["Property"] = { "=" : pv["PropertyPath"]};
                }
                if (pv["Property"] == "Descending") {
                  ae["Descending"] = pv["Bool"];
                }
              }
              delete ae["$Record"];
            }
        ...
        // TNT - RequestAtLeastSimplification
        //   remove unnecessary nesting
        if (aNameWithoutQualifier == "@UI.PresentationVariant.RequestAtLeast" &&
            (options && options.tntFlavor && !options.tntFlavor.skipAnnosRequestAtLeastSimplification)) {
          // for all array elements: replace element by its $PropetyPath property
          a.forEach( function(x, i, A) {
            if (x["$PropertyPath"]) A[i] = x["$PropertyPath"]; });
        }

        // TNT - UILineItemExpansion
        //   annotation is structured according to vocabulary, but in the csn a simple value is provided
        // TNT - UIImportanceDefaulting
        //   always set UI.Importance to High if not specified explicitly
        if (aNameWithoutQualifier == "@UI.LineItem") {
          for (let i in a) {
            let ae = a[i];
            if (ae["="] != undefined &&
                (options && options.tntFlavor && !options.tntFlavor.skipAnnosUILineItemExpansion)) {
              a[i] = { "Value": { "=": ae["="] } };
            }
            if (a[i]["@UI.Importance"] == undefined &&
                (options && options.tntFlavor && !options.tntFlavor.skipAnnosUIImportanceDefaulting)) {
              a[i]["@UI.Importance"] = { "#": "High" };
            }
          }
        }

        // TNT - FieldGroupValueExpansion
        //   replace shortcut form for property "Value" by full form
        // TNT - FieldGroupTypeCleansing
        //   replace properties "Type" by correct form "$Type"
        if (aNameWithoutQualifier == "@UI.FieldGroup") {
          for (let i in a) {
            let ae = a[i];
            if (ae["="] != undefined &&
                (options && options.tntFlavor && !options.tntFlavor.skipAnnosFieldGroupValueExpansion)) {
              a[i] = { "Value": { "=": ae["="] } }
            }
          }
        }
  ...
  */
  function moveAnnotationsFromAssocToForeignKeys() {
    ...
    glue.foreach(csn.definitions, obj => obj.kind == 'entity', entity => {
