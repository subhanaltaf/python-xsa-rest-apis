...
{
  static
  create(v, ns, alias, annotations=[], withEntityContainer=true)
  {
  ...
  innerXML(indent, what)
  {
    ...
    if(what=='annotations' || what=='all')
    {
      if(this._annotations.length > 0)
        this._annotations.forEach(a => xml += a.toXML(indent) + '\n');
    }
    return xml;
  }
  ...
  toJSONchildren(json)
  {
    ...
    if(this._annotations.length > 0)
    {
      ...
      this._annotations.forEach(a => json_Annotations[a.Target] = a.toJSON());
...
class EDM extends Node
{
  ...
  setDefaultReferences()
  {
    ...
    {
      let r = Reference.create(this._v, { Uri : "http://docs.oasis-open.org/odata/odata/v4.0/errata03/os/complete/vocabularies/Org.OData.Core.V1.xml" });
      r.append(Include.create(this._v, {Alias : "Core", Namespace : "Org.OData.Core.V1"} ))
      this._defaultRefs.push(r);

      r = Reference.create(this._v, { Uri : "http://docs.oasis-open.org/odata/odata/v4.0/errata03/os/complete/vocabularies/Org.OData.Measures.V1.xml" });
      r.append(Include.create(this._v, {Alias : "Measures", Namespace : "Org.OData.Measures.V1"} ))
      this._defaultRefs.push(r);

      r = Reference.create(this._v, { Uri : "http://docs.oasis-open.org/odata/odata/v4.0/errata03/os/complete/vocabularies/Org.OData.Capabilities.V1.xml" });
      r.append(Include.create(this._v, {Alias : "Capabilities", Namespace : "Org.OData.Capabilities.V1"} ))
      this._defaultRefs.push(r);

      r = Reference.create(this._v, { Uri : "http://docs.oasis-open.org/odata/odata-data-aggregation-ext/v4.0/cs02/vocabularies/Org.OData.Aggregation.V1.xml" });
...
class TypeBase extends Node 
{
  ...
  toJSONattributes(json)
  {
    ...
    if(this._type != 'Edm.String')   // Edm.String is default)
...
class PropertyBase extends TypeBase {
  static
  create(v, attributes, csn, parentCsn)
  {
    let node = super.create(v, attributes, csn);
    node.set({ _csn: csn, _parentCsn: parentCsn });
    if(node.v2)
    {
      let typecsn = csn.items || csn;
  ...
  setNullable(csn)
  {
    ...
    if((csn.key && !(csn.notNull === false)) || csn.notNull == true)
    {
      this.Nullable = false;
    }
  }
...
class Property extends PropertyBase {
  ...
  create(v, attributes, csn, parentCsn) 
  {
    ...
    let node = super.create(v, attributes, csn, parentCsn);
    // TIPHANACDS-4180
    if(node.v2)
    {
...
{
  static
    create(v, attributes, csn={}, parentCsn={}, mode=null)
    {
      let node = super.create(v, attributes, csn, parentCsn);
      if(mode != null)
        node.Mode = mode;
...
class NavigationProperty extends Property {
  static
  create(v, attributes, csn, parentCsn, v4)
  {
    NavigationProperty.DOLLAR_SELF = '$self'

    let navProp = super.create(v, attributes, csn, parentCsn);
    navProp.set( { _type: attributes.Type, _isCollection: glue.isToMany(csn) } );
    if (v4 && navProp._isCollection)
      navProp.Type = `Collection(${attributes.Type})`

    if(csn.containsTarget)
      navProp.ContainsTarget = csn.containsTarget;
  ...
  {
    glue.forAll(this.getReferentialConstraints().constraints,
  ...
  getReferentialConstraints()
  {
    ...
    if(assocCsn.onCond)
    {
      ...
      if(result.constraints[NavigationProperty.DOLLAR_SELF])
      {
        ...
        {
          if(originAssocCsn.onCond)
          {
            /*
            entity E  {
              key id : Integer;
              toE: association to E on id = toE.id;
              toMe: associaton to E on toMe.toE = $self; };
            */
            throw 'No foreign keys found for underlying source association "' + originAssocCsn.name +
                  '" of backlink association "' + assocCsn.name + '", but has ON condition: ' + originAssocCsn.onCond;
          }
          ...
          // association to the two corresponding NavigationProperties
    ...
    function determineMultiplicity()
    {
      ...
      */
      ...
      if(assocCsn.cardinality)
      {
        srcCardinality = (assocCsn.cardinality.sourceMax == 1) ? '0..1' : '*';
        tgtCardinality = (assocCsn.cardinality.targetMax > 1 || assocCsn.cardinality.targetMax == '*') ? '*' :
                        (assocCsn.cardinality.targetMin == 1 && assocCsn.cardinality.targetMax == 1) ? '1' : '0..1';
      }

      result.multiplicity = [srcCardinality, tgtCardinality];
    }
  }
}
