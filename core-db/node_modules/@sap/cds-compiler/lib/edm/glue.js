...
function isAssociation(artifact) {
  ...
  // FIXME: Different representations possible: we simply look for target here
...
function initializeModel(model, options) 
{
  ...
  foreach(model.definitions, a => isStructuredArtifact(a), initializeStructure);
  ...
  function initializeStructure(struct) {
    ...
    forAll(struct.elements, (element, elementName) => {
      ...
      // Resolve association (if any)
      if (isAssociationOrComposition(element)) {
        initializeAssociation(element, struct);
      }
    });
  ...
  function initializeAssociation(element, struct) 
  {
