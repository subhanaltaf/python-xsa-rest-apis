...
function fns( model, environment = (o => o.artifacts || Object.create(null) ) ) {
  ...
  const specExpected = {
    annotation: { unchecked: true },
    extension: {},                   // while collecting extend/annotate (path root)
    extend: {},                      // for giving error for non-applied EXTEND
    annotate: { severity: 'Info' }, // for giving info message for non-applied ANNOTATE
  ...
  function resolveUncheckedPath( ref, expected, env ) {
    ...
    if (!art)
      return (spec.unchecked) ? pathName( ref.path ) : undefined;
  ...
  function resolvePath( ref, expected, user, env, extDict ) {
    ...
    if( ref.absolute && !ref.path ) { // TODO: this should disappear
      ...
      if (!art)
        signal( error`Artifact '${refString(ref)}' has not been found`, ref.location );
      return art;
    }
  ...
  function getPathRoot( path, spec, env, extDict ) {
    if (!spec.next && !extDict)
      extDict = (env.$frontend === 'json' || env.$frontend === 'i18n' || env.$frontend === 'xml')
                ? model.definitions
                : model.$builtins;
    ...
    if (env.name && env.name.absolute)
      // TODO: be better for elements (when blockProp has been used)
      signalNotFound( `Name "${head.id}" has not been found from "${env.name.absolute}"`,
                      head.location, valid, spec.severity );
    else if (env.kind === 'source')
      signalNotFound( error`Name '${head.id}' has not been found at top-level`,
                      head.location, valid, spec.severity );
    else
      signalNotFound( error`Name '${head.id}' has not been found from here`,
                      head.location, valid, spec.severity );
    return undefined
  }
  ...
  function getPathItem( tail, spec, art, head ) {
    for (let item of tail) {
      ...
      else if (art.kind === 'context' || art.kind == 'service' || art.kind == 'namespace' ) {
        signalNotFound( error`Artifact '${art.name.absolute + '.' + item.id}' has not been found`,
                        combinedLocation( head, item ), [env], spec.severity );
        return undefined;
      }
      else if (art.kind !== 'element') {
        signalNotFound( error`Element '${item.id}' has not been found in '${art.name.absolute}'`, //  || JSON.stringify(art.name)}'
                        item.location, [env], spec.severity );
        return undefined;
      }
      else {
        signalNotFound( `Element "${item.id}" has not been found in element "${art.name.element||art.name.alias}" of "${art.name.absolute}"`,
                        item.location, [env], spec.severity );
        return undefined;
      }
    }
...
function linkToOrigin( origin, name, parent, prop, location ) {
  let elem = {
    ...
    kind: origin.kind,
    origin: Object.assign( {}, origin.name ),
    location
  };
  if (parent)
