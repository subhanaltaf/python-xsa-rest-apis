...
function define( model, options = model.options || {} ) {
  ...
  function checkRedefinitions( obj, name, prop, i ) {
    ...
    if (prop === 'elements')
      message( `Duplicate definition of element "${name}"`, loc );
    else if (prop === 'enum')
      message( `Duplicate definition of enum "${name}"`, loc );
    else if (prop === 'foreignKeys')
      message( `Duplicate definition of foreign key "${name}"`, loc );
    else if (prop === 'actions')
      message( `Duplicate definition of action or function "${name}"`, loc );
    else if (prop === 'params')
      message( `Duplicate definition of parameter "${name}"`, loc );
    else if (obj.kind === 'namespace')
      message( `Other definition blocks "${name}" for namespace name`, loc );
    else
      message( `Duplicate definition of artifact "${name}"`, loc );
  }
  ...
  function initSource( src ) {
    ...
    if (!namespace) {
      ...
      block.artifacts[ last.id ] = { // assert( last.id === namespace.name.id )
        ...
        //implicit: true,
        name: { id: last.id, absolute, location: last.location, calculated: true },
        extern: decl,
  ...
  function initUsings( src ) {  // TODO: remove if old forHana() is discontinued
    if (!src.usings)
      return;
    if (!src.artifacts)
    ...
    for (let def of src.usings) {
      let ue = def.extern;
      if (!def.name) {
        def.name = Object.assign( { calculated: true }, ue.path[ ue.path.length - 1] );
      }
      ...
      addToDict( src.artifacts, def.name.id, def, function( name, loc ) {
        message( `Duplicate definition of top-level name "${name}"`, loc );
      } );
    }
  }
  ...
  function initArtifacts( construct, parent, block, collectExts,
    ...
    function init( art ) {
      if (art.pathPrefix || art.kind === 'using') // real.path as name, or using proxies
        return;
      if (kindProperties[ art.kind ].artifacts && !defProp) { // context or service, not from JSON
        let env = art;
        ...
        for (let q of art.queries) {
          let obj = (q === art._leadingQuery) ? art : q; // TODO: always use q
          initMembers( obj, obj, block );
        }
    ...
    function initDollarSelf( art ) {
      ...
      if ((options.tntFlavor && !options.tntFlavor.skipOldStyleSelf) || options.oldstyleSelf)
  ...
  function initQueryExpression( art, query, parents = [] ) {
    ...
    if (query.path) {           // path in FROM - this query is a table ref
      ...
      if (!query.name) {
        ...
        query.name = { calculated: true, id: last.id, location: last.location };
      }
    ...
    function signalDuplicate( name, loc ) {
      message( `Duplicate definition of table alias or mixin "${name}"`, loc );
    }
      ...
      for (let name of extNames) {
        ...
        if (!art) {
          if (!options.lintMode) {
            ...
            art = { kind: 'annotate', name: { absolute: name } };
  ...
  function extendMembers( extensions, art ) {
    ...
    ['elements', 'actions', 'enum'].forEach( function ext(prop) {
      let dict = art._extend && art._extend[prop];
      for (let name in dict) {
        let member = art[prop][name];
        if (!member)
