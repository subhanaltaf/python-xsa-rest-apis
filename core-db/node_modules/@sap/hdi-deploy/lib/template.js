...
exports.convertTemplate = function(template, services) {
    ...
    function replaceKey(target, key) {
        ...
        if (key === 'grantor') {
            ...
            var schema = services.getCreds(grantor).schema;
        ...
        var newKey = key.substr(0, key.length - 10); // everything but '.configure'
        if (newKey === '*') {
            grantor = target[key];
            var creds = services.getCreds(grantor);
            ...
            var replacement = target[key].split('/');    // <grantor>/<value>
            grantor = replacement[0];
            var value = replacement[1];
